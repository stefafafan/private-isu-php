...
## PHP 8.3に戻してプロファイラ実行せずにベンチ回してみる: 431496点

```sh
{"pass":true,"score":431496,"success":417489,"fail":0,"messages":[]}
```

| COUNT  | 1XX |  2XX  |  3XX   | 4XX  | 5XX | METHOD |        URI         |  MIN  |  MAX  |   SUM   |  AVG  |  P90  |  P95  |  P99  | STDDEV | MIN(BODY) |  MAX(BODY)  |   SUM(BODY)    | AVG(BODY) |
|--------|-----|-------|--------|------|-----|--------|--------------------|-------|-------|---------|-------|-------|-------|-------|--------|-----------|-------------|----------------|-----------|
| 13887  | 0   | 13887 | 0      | 0    | 0   | GET    | /                  | 0.001 | 0.058 | 118.731 | 0.009 | 0.012 | 0.013 | 0.016 | 0.003  | 2176.000  | 5019.000    | 33896377.000   | 2440.871  |
| 10267  | 0   | 10267 | 0      | 0    | 0   | GET    | ^/posts/\d+        | 0.001 | 0.042 | 83.234  | 0.008 | 0.011 | 0.013 | 0.016 | 0.003  | 629.000   | 1920.000    | 12226282.000   | 1190.833  |
| 5020   | 0   | 5020  | 0      | 0    | 0   | GET    | /posts             | 0.002 | 0.042 | 56.967  | 0.011 | 0.015 | 0.016 | 0.019 | 0.003  | 4465.000  | 5222.000    | 24059194.000   | 4792.668  |

```sql
# Rank Query ID                            Response time Calls R/Call V/M 
# ==== =================================== ============= ===== ====== ====
#    1 0x6043518FB89B720C84648F90F3DA0885  15.1892 27.9% 31217 0.0005  0.00 SELECT comments
#    2 0x009A61E5EFBD5A5E4097914B4DBD1C07  12.5962 23.1%   472 0.0267  0.02 INSERT posts
#    3 0x5C29F616FBA5526D7150C7847FA60150  11.6518 21.4% 23583 0.0005  0.00 SELECT users
...
SELECT * FROM `comments` WHERE `post_id` IN ('8277','7530','7324','7004','6366','6333','4803','4515','3356','2469','1222','798','12') ORDER BY `created_at` DESC\G
INSERT INTO `posts` (`user_id`, `mime`, `imgdata`, `body`) VALUES...
SELECT * FROM `users` WHERE `id` IN ('587','742','545','387','297','647','633','844','197','879','586','68','383','865','62','822','874','223','49','795'/*... omitted 202 items ...*/)\G
```

## PHP 8.3に戻してプロファイラを再び実行する: 344578点

```sh
{"pass":true,"score":344578,"success":332876,"fail":0,"messages":[]}
```

| COUNT  | 1XX |  2XX  |  3XX   | 4XX  | 5XX | METHOD |        URI         |  MIN  |  MAX  |   SUM   |  AVG  |  P90  |  P95  |  P99  | STDDEV | MIN(BODY) |  MAX(BODY)  |   SUM(BODY)    | AVG(BODY) |
|--------|-----|-------|--------|------|-----|--------|--------------------|-------|-------|---------|-------|-------|-------|-------|--------|-----------|-------------|----------------|-----------|
| 11224  | 0   | 11224 | 0      | 0    | 0   | GET    | /                  | 0.001 | 0.055 | 121.954 | 0.011 | 0.016 | 0.017 | 0.022 | 0.004  | 2139.000  | 5018.000    | 27364159.000   | 2438.004  |
| 8136   | 0   | 8136  | 0      | 0    | 0   | GET    | ^/posts/\d+        | 0.001 | 0.045 | 85.012  | 0.010 | 0.015 | 0.017 | 0.021 | 0.004  | 632.000   | 1872.000    | 9676150.000    | 1189.301  |
| 3990   | 0   | 3990  | 0      | 0    | 0   | GET    | /posts             | 0.003 | 0.056 | 57.449  | 0.014 | 0.019 | 0.021 | 0.027 | 0.004  | 4461.000  | 5309.000    | 19132641.000   | 4795.148  |

```sql
# Rank Query ID                            Response time Calls R/Call V/M 
# ==== =================================== ============= ===== ====== ====
#    1 0x6043518FB89B720C84648F90F3DA0885  14.3432 29.4% 24968 0.0006  0.00 SELECT comments
#    2 0x009A61E5EFBD5A5E4097914B4DBD1C07  10.5058 21.5%   428 0.0245  0.02 INSERT posts
#    3 0x5C29F616FBA5526D7150C7847FA60150  10.1366 20.8% 17469 0.0006  0.00 SELECT users
SELECT * FROM `comments` WHERE `post_id` IN ('9948','9947','9946','9945','9944','9943','9942','9941','9940','9939','9938','9937','9936','9935','9934','9933','9932','9931','9930','9929') ORDER BY `created_at` DESC\G
INSERT INTO `posts` (`user_id`, `mime`, `imgdata`, `body`) VALUES...
SELECT * FROM `users` WHERE `id` IN ('132','744','241','734','868','39','772','273','898','45','532','145','611','740','528','136','711','116','446','898'/*... omitted 176 items ...*/)\G
```

![Flamegraph](./images/202412040932-flamegraph.svg)

## PDO::ATTR_PERSISTENT を設定する: 371402点

実施したこと: [0998abe](https://github.com/stefafafan/private-isu-php/commit/0998abe8717a8b24a0116b00eab449e82e2c8527)

```sh
{"pass":true,"score":371402,"success":358184,"fail":0,"messages":[]}
```

| COUNT  | 1XX |  2XX  |  3XX   | 4XX  | 5XX | METHOD |        URI         |  MIN  |  MAX  |   SUM   |  AVG  |  P90  |  P95  |  P99  | STDDEV | MIN(BODY) |  MAX(BODY)  |   SUM(BODY)    | AVG(BODY) |
|--------|-----|-------|--------|------|-----|--------|--------------------|-------|-------|---------|-------|-------|-------|-------|--------|-----------|-------------|----------------|-----------|
| 12408  | 0   | 12408 | 0      | 0    | 0   | GET    | /                  | 0.001 | 0.042 | 110.179 | 0.009 | 0.013 | 0.015 | 0.020 | 0.004  | 2198.000  | 22372.000   | 30126330.000   | 2427.976  |
| 9051   | 0   | 9051  | 0      | 0    | 0   | GET    | ^/posts/\d+        | 0.001 | 0.039 | 77.324  | 0.009 | 0.012 | 0.014 | 0.020 | 0.003  | 632.000   | 1874.000    | 10809592.000   | 1194.298  |
| 4210   | 0   | 4210  | 0      | 0    | 0   | GET    | /posts             | 0.003 | 0.055 | 51.410  | 0.012 | 0.017 | 0.019 | 0.024 | 0.004  | 4461.000  | 5190.000    | 20129924.000   | 4781.455  |

```sql
# Rank Query ID                            Response time Calls R/Call V/M 
# ==== =================================== ============= ===== ====== ====
#    1 0x6043518FB89B720C84648F90F3DA0885  14.2170 28.1% 27569 0.0005  0.00 SELECT comments
#    2 0x009A61E5EFBD5A5E4097914B4DBD1C07  12.1879 24.1%   457 0.0267  0.02 INSERT posts
#    3 0x5C29F616FBA5526D7150C7847FA60150  10.3244 20.4% 19128 0.0005  0.00 SELECT users
...
SELECT * FROM `comments` WHERE `post_id` IN ('9949','9948','9947','9946','9945','9944','9943','9942','9941','9940','9939','9938','9937','9936','9935','9934','9933','9932','9931','9930') ORDER BY `created_at` DESC\G
INSERT INTO `posts` (`user_id`, `mime`, `imgdata`, `body`) VALUES...
SELECT * FROM `users` WHERE `id` IN ('981','198','529','131','295','770','882','915','53','490','503','118','685','869','722','879','819','835','542','759'/*... omitted 202 items ...*/)\G
```

![Flamegraph](./images/202412040941-flamegraph.svg)

## composer autoloader optimize 設定する: 370456点

実施したこと: [6d753a5](https://github.com/stefafafan/private-isu-php/commit/6d753a5d79ceec23ecde711653b1a2e4fd4fd31f)

```sh
{"pass":true,"score":370456,"success":357261,"fail":0,"messages":[]}
```

| COUNT  | 1XX |  2XX  |  3XX   | 4XX  | 5XX | METHOD |        URI         |  MIN  |  MAX  |   SUM   |  AVG  |  P90  |  P95  |  P99  | STDDEV | MIN(BODY) |  MAX(BODY)  |   SUM(BODY)    | AVG(BODY) |
|--------|-----|-------|--------|------|-----|--------|--------------------|-------|-------|---------|-------|-------|-------|-------|--------|-----------|-------------|----------------|-----------|
| 12342  | 0   | 12342 | 0      | 0    | 0   | GET    | /                  | 0.001 | 0.036 | 112.646 | 0.009 | 0.014 | 0.015 | 0.021 | 0.004  | 2193.000  | 5019.000    | 30109128.000   | 2439.566  |
| 8973   | 0   | 8973  | 0      | 0    | 0   | GET    | ^/posts/\d+        | 0.001 | 0.037 | 78.497  | 0.009 | 0.013 | 0.015 | 0.020 | 0.004  | 632.000   | 1866.000    | 10643764.000   | 1186.199  |
| 4230   | 0   | 4230  | 0      | 0    | 0   | GET    | /posts             | 0.002 | 0.037 | 52.070  | 0.012 | 0.017 | 0.019 | 0.025 | 0.004  | 4486.000  | 5221.000    | 20322991.000   | 4804.490  |

```sql
# Rank Query ID                            Response time Calls R/Call V/M 
# ==== =================================== ============= ===== ====== ====
#    1 0x6043518FB89B720C84648F90F3DA0885  14.1153 28.3% 27416 0.0005  0.00 SELECT comments
#    2 0x009A61E5EFBD5A5E4097914B4DBD1C07  11.5694 23.2%   463 0.0250  0.02 INSERT posts
#    3 0x5C29F616FBA5526D7150C7847FA60150  10.3819 20.8% 19978 0.0005  0.00 SELECT users
...
SELECT * FROM `comments` WHERE `post_id` IN ('9946','9945','9944','9943','9942','9941','9940','9939','9938','9937','9936','9935','9934','9933','9932','9931','9930','9929','9928','9926') ORDER BY `created_at` DESC\G
INSERT INTO `posts` (`user_id`, `mime`, `imgdata`, `body`) VALUES...
SELECT * FROM `users` WHERE `id` IN ('397','465','58','649','32','920','470','55','801','260','648','73','463','493','766','691','823')\G
```

![Flamegraph](./images/202412040957-flamegraph.svg)

## Unix domain socket を使うようにする: 377267点

実施したこと: [9cd406f](https://github.com/stefafafan/private-isu-php/commit/9cd406fb26a138c24cf4e153260f26876213f50f)

```sh
{"pass":true,"score":377267,"success":363764,"fail":0,"messages":[]}
```

| COUNT  | 1XX |  2XX  |  3XX   | 4XX  | 5XX | METHOD |        URI         |  MIN  |  MAX  |   SUM   |  AVG  |  P90  |  P95  |  P99  | STDDEV | MIN(BODY) |  MAX(BODY)  |   SUM(BODY)    | AVG(BODY) |
|--------|-----|-------|--------|------|-----|--------|--------------------|-------|-------|---------|-------|-------|-------|-------|--------|-----------|-------------|----------------|-----------|
| 12666  | 0   | 12666 | 0      | 0    | 0   | GET    | /                  | 0.001 | 0.041 | 110.089 | 0.009 | 0.013 | 0.015 | 0.020 | 0.004  | 2155.000  | 5021.000    | 30549036.000   | 2411.893  |
| 9141   | 0   | 9141  | 0      | 0    | 0   | GET    | ^/posts/\d+        | 0.001 | 0.046 | 76.132  | 0.008 | 0.013 | 0.014 | 0.019 | 0.004  | 632.000   | 1841.000    | 10878728.000   | 1190.103  |
| 4270   | 0   | 4270  | 0      | 0    | 0   | GET    | /posts             | 0.002 | 0.052 | 51.054  | 0.012 | 0.017 | 0.018 | 0.024 | 0.004  | 4450.000  | 5347.000    | 20627880.000   | 4830.885  |

```sql
# Rank Query ID                            Response time Calls R/Call V/M 
# ==== =================================== ============= ===== ====== ====
#    1 0x6043518FB89B720C84648F90F3DA0885  14.6001 28.7% 27975 0.0005  0.00 SELECT comments
#    2 0x009A61E5EFBD5A5E4097914B4DBD1C07  11.5057 22.6%   468 0.0246  0.02 INSERT posts
#    3 0x5C29F616FBA5526D7150C7847FA60150  10.7365 21.1% 20006 0.0005  0.00 SELECT users
...
SELECT * FROM `comments` WHERE `post_id` IN ('9961','9960','9958','9957','9956','9955','9954','9953','9952','9951','9950','9949','9948','9947','9946','9945','9944','9943','9942','9941') ORDER BY `created_at` DESC\G
INSERT INTO `posts` (`user_id`, `mime`, `imgdata`, `body`) VALUES...
SELECT * FROM `users` WHERE `id` IN ('737','397','607','527','779','427','823','407','340','902','573','394','137','646','242','544','671','192','208','90'/*... omitted 186 items ...*/)\G
```

![Flamegraph](./images/202412041012-flamegraph.svg)

## 各種パラメータ調整、PHP8.4に戻す、ログを無効化: 533117点

実施したこと: [ade59b1](https://github.com/stefafafan/private-isu-php/commit/ade59b123a978098c5f43b8a483661229f9f12ef), [27a4b72](https://github.com/stefafafan/private-isu-php/commit/27a4b727d398653b8701c2e050fb6abd668950cb), [3491f22](https://github.com/stefafafan/private-isu-php/commit/3491f222eae4548925f480f251689309c9a6a0d3), [e791ac4](https://github.com/stefafafan/private-isu-php/commit/e791ac42a4e5c0364766a914fa5059c700adaf23), [ef9d289](https://github.com/stefafafan/private-isu-php/commit/ef9d28917aac9439cc553ecaa5df7101435ff184)

```sh
{"pass":true,"score":533117,"success":517830,"fail":0,"messages":[]}
```

## MySQL 8.0 → 8.4 へアップグレード: 502465点
```sh
{"pass":true,"score":502465,"success":484742,"fail":0,"messages":[]}
```

## MySQL 8.4 → 9.1 へアップグレード: 498680点
```sh
{"pass":true,"score":498680,"success":481675,"fail":0,"messages":[]}
```
