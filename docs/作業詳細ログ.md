...
```sh
mkdir -p ./etc/nginx/
rsync -a isu01:/etc/nginx/nginx.conf ./etc/nginx/
```
...
rsync -a isu01:/etc/nginx/sites-enabled/isucon-php.conf ./etc/nginx/sites-enabled
```
...
rsync -a isu01:/etc/nginx/sites-available/isucon-php.conf ./etc/nginx/sites-available
```
